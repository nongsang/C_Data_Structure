...
// 92 ~ 106행
// 처음
// r = q은 둘다 NULL이기 때문에 변경된 것이 없다.
// q = p를 하여 q는 p와 같이 head를 가리킨다.
// p = p->next를 하여 p는 다음 노드를 가리킨다.
// q->next = r을 하여 맨 처음 노드는 다음을 가리키는 노드를 없게 한다.
// 두번째
// r = q을 하여 r이 q가 가리키던 노드를 가리키게 한다. -> 처음 노드를 r이 가리키게 된다.
// q = p를 하여 q는 p가 가리키던 노드를 가리키게 한다. -> q는 r 다음노드인 p를 가리킨다.
// p = p->next를 하여 p는 다음 노드를 가리킨다.
// q->next = r를 하여 q의 다음 노드는 맨 처음 노드를 가리키게 된다.
// 이후 반복
